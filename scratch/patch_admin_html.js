...
function main() {
    console.log("Patching admin.html using Node.js with normalized newlines...");
    ...
    const old_body_style = "        body { background: var(--dark-bg); }";
    const new_body_style = `        body {
            margin: 0;
            padding: 0;
    ...
    // 2. Add toggleSidebar helper inside head
    const old_head_closing = "</head>";
    const new_head_closing = `
    <script>
    ...
    content = content.replace(old_head_closing, new_head_closing);

    // 3. Replace horizontal tabs
    const old_tabs_block = `        <!-- Tabs -->
    ...
    const old_container_header = `    <div class="app-container" style="max-width:860px; padding-bottom:30px;">
    ...
    const new_container_header = `    <div class="admin-wrapper">
    ...
    const old_closing = `        </main>\n    </div>`;
    const new_closing = `        </main>\n        </div> <!-- Closing main-content -->\n    </div> <!-- Closing admin-wrapper -->`;
